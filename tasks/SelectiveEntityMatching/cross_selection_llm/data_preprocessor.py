...
class DataPreprocessor:
    ...
    def create_input_text_and_label(self, sample):
        ...
        """
        instruction = "Instruct: Given a query entity, retrieve a similar entity that semantically matches the query from the candidates. \nQuery: "
        ...
        # Concatenate the query and candidates with the special token
        special_token = self.tokenizer.bos_token  # e.g., "<s>"
        concatenated_text = text_query

        for candidate in shuffled_candidates:
            concatenated_text += f" {special_token} {candidate}"

        if self.input_type == 'text_on':
            sample['text'] = concatenated_text
        elif self.input_type == 'inst_text_on':
            sample['text'] = instruction + ' ' + concatenated_text
        ...
        sample['input_ids_text'] = encodings["input_ids"]
        sample['attention_mask_text'] = encodings["attention_mask"]
...
class DataPreprocessor_pairwise:
    ...
    def preprocess_data(self, dataset, label2id, tokenizer, max_length=1024, input_type='text_st_on', has_label=True, remove_unused_col=False, is_train:bool=True):
        ...
        remove_cols = [n for n in dataset.column_names if n not in ["text","labels", "input_ids_text", "attention_mask_text"]]
        print(remove_cols)
        if remove_unused_col:
    ...
    def create_input_text_and_label(self, sample, input_type):
        ...
        special_token = self.tokenizer.bos_token

        if input_type == 'text_st_on':
            sample["text"] = f"<entity1> {text_src} </entity1> {special_token} <entity2> {text_tgt} </entity2>"
        elif input_type == 'text_on':
            sample["text"] = text_src + special_token + text_tgt
        elif input_type == 'inst_text_st_on':
            sample["text"] = instruction + '\n ' + f"<entity1> {text_src} </entity1>" + special_token + f"<entity2> {text_tgt} </entity2>"
        elif input_type == 'inst_text_on':
            sample["text"] = instruction + '\n ' + text_src + special_token + text_tgt
