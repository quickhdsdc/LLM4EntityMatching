...
PROMPT_ST_DIC = {
    'nl': [INSTRUCTION_KEY, INSTRUCTION_KEY_END, INPUT_KEY, INPUT_KEY_END, ENTITY1, ENTITY1_END, ENTITY2, ENTITY2_END,
           RESPONSE_KEY, END_KEY],
    'st': [INSTRUCTION_KEY_ST, INSTRUCTION_KEY_END_ST, INPUT_KEY_ST, INPUT_KEY_END_ST, ENTITY1_ST, ENTITY1_END_ST,
           ENTITY2_ST, ENTITY2_END_ST, RESPONSE_KEY_ST, END_KEY_ST]}
...
class DataPreprocessor_pairwise:
    ...
    def preprocess_data(self, model, dataset, args=None, is_train: bool = True, prompt_type: str = 'pt0',
                        prompt_st_type: str = 'nl'):
        """
        ...
        instruction_key, instruction_key_end, input_key, input_key_end, old_start, entity1_end, entity2, entity2_end, response_key, end_key = \
        PROMPT_ST_DIC[self.prompt_st_type]
        seed = 42
        ...
        remove_cols = [n for n in dataset.column_names if n not in ["text", "label", "instr", "input"]]
    ...
    def create_task_prompt(self, prompt_type):
        ...
        if prompt_type == 'pt0':
            ...
            EXAMPLES = self.create_examples(2, self.dataset)
            p = TASK_PROMPT0 + "\n" + EXAMPLES
    ...
    def create_examples(self, n_sample, dataset):
        ...
        """
        pos_ds = dataset.filter(lambda example: example['label'] in ['yes', 1])
        neg_ds = dataset.filter(lambda example: example['label'] in ['no', 0])
    ...
    def create_prompt_formats_train(self, sample):
        ...
        """
        instruction_key, instruction_key_end, input_key, input_key_end, entity1, entity1_end, entity2, entity2_end, response_key, end_key = \
        PROMPT_ST_DIC[self.prompt_st_type]
        ...
        input_parts = [part for part in [input_context, response, end] if part]
        input = "\n".join(input_parts)
        sample["input"] = input
        return sample
    ...
    def create_prompt_formats_test(self, sample):
        ...
        sample["text"] = formatted_prompt
        sample["instr"] = instruction
        input_parts = [part for part in [input_context, response] if part]
        input = "\n".join(input_parts)
        sample["input"] = input
        return sample
