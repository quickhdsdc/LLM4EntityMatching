...
class Evaluater:
    ...
    def load_model(self, finetuned_model_dir:Path, labels, label2id, id2label):
        tokenizer = RobertaTokenizer.from_pretrained(str(finetuned_model_dir))
        tokenizer.pad_token = tokenizer.eos_token
        tokenizer.padding_side = 'right'
        compute_dtype = getattr(torch, "float16")   
        model = RobertaForSequenceClassification.from_pretrained(str(finetuned_model_dir),num_labels=len(labels))
    ...
    def evaluate(self, test, labels, label2id, id2label, model_dir, max_length=512, output_dir=None, model=None,
        ...
        """
        if flag_fine_tuning:
            model, tokenizer = self.load_model(model_dir, labels, label2id, id2label)
